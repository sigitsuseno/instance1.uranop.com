...
function buildNormalSlipData(r) {
  ...
  return { isSplit: false, employeeName: r.employee_name, employeeCode: r.employee_code, periodName: selectedPeriod.value?.name || '', tanggalPenggajian: selectedPeriod.value?.tanggal_penggajian || null, remainingLeave: r.remaining_leave || 0, gajiPokok: r.gaji_pokok, ratePerHari: Math.round(ratePerHari), hkDays: hk, hkAmount: Math.round(hkAmount), tjMasaKerja: r.tj_masa_kerja, tunjangan: r.tunjangan, premiHadir: r.premi_hadir, overtimeHours: (r.lm_count || 0) + (r.lembur_count || 0), overtimeAmount: r.upah_lembur, revisi: r.revisi || 0, subTotalEarnings: Math.round(subTotalEarnings), bpjsTK: r.bpjs_tk, bpjsKes: r.bpjs_ks, bpjsPensiun: r.bpjs_pen, pph: r.pph, cashbon: r.cashbon, deductDays: deductDays, potonganKehadiran: potKehadiran, pblt: r.pblt, subTotalDeduction: Math.round(subTotalDeduction), totalTerima: r.gaji_bersih }
}
...
function buildSplitSlipData(r) {
  ...
  return { isSplit: true, employeeName: r.employee_name, employeeCode: r.employee_code, tanggalPenggajian: selectedPeriod.value?.tanggal_penggajian || null, remainingLeave: r.remaining_leave || 0, part1, part2, totalTerima: (part1?.totalTerima || 0) + (part2?.totalTerima || 0) }
}
