          ...
          <template #item.actions="{ item }">
            <div class="flex items-center gap-1">
              <BaseButton variant="ghost" size="sm" @click="openLeaveTypeForm(item)">
                <i class="bx bx-pencil w-4 h-4"></i>
              </BaseButton>
              <BaseButton variant="ghost" size="sm" @click="deleteLeaveType(item)">
                <i class="bx bx-trash w-4 h-4 text-red-500"></i>
              </BaseButton>
            </div>
          </template>
        </DataTable>
...
const fetchData = async () => {
  ...
  try {
    const [resTypes, resPolicies, resPeriods] = await Promise.all([
      api.get('/api/v1/supervisor/master/leave-settings/types'),
      api.get('/api/v1/supervisor/master/leave-settings/policies'),
      api.get('/api/v1/supervisor/master/leave-settings/periods')
    ])
    leaveTypes.value = resTypes.data || []
    policies.value = resPolicies.data || []
    periods.value = resPeriods.data || []
  } catch (error) {
    alert('Gagal memuat data pengaturan cuti.')
  } finally {
...
async function saveLeaveType() {
  ...
  try {
    if (editingLeaveType.value) {
      await api.put(`/api/v1/supervisor/master/leave-settings/types/${editingLeaveType.value.id}`, leaveTypeForm)
    } else {
      await api.post('/api/v1/supervisor/master/leave-settings/types', leaveTypeForm)
    }
    showLeaveTypeModal.value = false
    fetchData()
  } catch (error) {
    alert('Gagal menyimpan tipe cuti')
  } finally {
...
function openPolicyForm(item) {
  if (item) {
    ...
    Object.assign(policyForm, {
        ...item,
        requires_one_year_service: Boolean(Number(item.requires_one_year_service)),
        can_carry_forward: Boolean(Number(item.can_carry_forward)),
    })
  } else {
...
async function savePolicy() {
  ...
  try {
    if (editingPolicy.value) {
      await api.put(`/api/v1/supervisor/master/leave-settings/policies/${editingPolicy.value.id}`, policyForm)
    } else {
      await api.post('/api/v1/supervisor/master/leave-settings/policies', policyForm)
    }
...
async function savePeriod() {
  ...
  try {
    if (editingPeriod.value) {
      await api.put(`/api/v1/supervisor/master/leave-settings/periods/${editingPeriod.value.id}`, periodForm)
    } else {
      await api.post('/api/v1/supervisor/master/leave-settings/periods', periodForm)
    }
...
async function confirmDeleteAction() {
  try {
    if (confirmDelete.type === 'leaveType') {
      await api.destroy(`/api/v1/supervisor/master/leave-settings/types/${confirmDelete.id}`)
    } else if (confirmDelete.type === 'policy') {
      await api.destroy(`/api/v1/supervisor/master/leave-settings/policies/${confirmDelete.id}`)
    } else if (confirmDelete.type === 'period') {
      await api.destroy(`/api/v1/supervisor/master/leave-settings/periods/${confirmDelete.id}`)
    }
    fetchData()
  } catch (error) {
