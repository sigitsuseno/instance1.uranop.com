...
async function fetchConfigs() {
  loading.value = true
  try {
    const res = await get('/api/v1/supervisor/master/bpjs-configs')
...
async function save() {
  ...
  try {
    if (isEdit.value) {
      await put(`/api/v1/supervisor/master/bpjs-configs/${editingId.value}`, form.value)
    } else {
      await post('/api/v1/supervisor/master/bpjs-configs', form.value)
    }
    closeModal()
    fetchConfigs()
...
async function toggleActive(c, active) {
  try {
    const url = `/api/v1/supervisor/master/bpjs-configs/${c.id}/${active ? 'activate' : 'deactivate'}`
    await post(url)
    fetchConfigs()
...
async function doDelete() {
  ...
  deleting.value = true
  try {
    await destroy(`/api/v1/supervisor/master/bpjs-configs/${deleteTarget.value.id}`)
