...
async function fetchData() {
  ...
  try {
    ...
    const res = await get(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan?${new URLSearchParams(params)}`)
...
function openEditModal(emp) {
  ...
  selectedEmployee.value = emp
  // Load full BPJS data
  get(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${emp.id}?bpjs_id=${emp.bpjs_id}`).then(res => {
...
async function toggleCheckbox(emp, field, event) {
  ...
  if (!emp.bpjs_id) {
    try {
      ...
      const res = await post(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${emp.id}`, payload)
  ...
  emp[field] = newVal
  try {
    await put(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${emp.id}`, { [field]: newVal, bpjs_id: emp.bpjs_id })
  } catch (e) {
    emp[field] = !newVal
    event.target.checked = !newVal
...
async function save() {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      await put(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${selectedEmployee.value.id}`, payload)
    } else {
      await post(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${selectedEmployee.value.id}`, payload)
    }
    closeModal()
    fetchData()
  } catch (e) {
...
async function doDelete() {
  ...
  deleting.value = true
  try {
    await destroy(`/api/v1/supervisor/employee-data/bpjs-karyawan/keanggotaan/${deleteTarget.value.id}?bpjs_id=${deleteTarget.value.bpjs_id}`)
