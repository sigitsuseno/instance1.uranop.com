...
const handleUpload = async () => {
  ...
  formData.append('year', props.activeYear)
  formData.append('month', props.activeMonth)

  try {
    const response = await fetch('/api/v1/supervisor/employee-data/karyawan-group/preview-import', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      periodName.value = result.period_name || ''
      step.value = 2
    } else {
...
const handleProcess = async () => {
  ...
  try {
    const response = await fetch('/api/v1/supervisor/employee-data/karyawan-group/import', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Accept': 'application/json',
    ...
    if (response.ok) {
      ...
      notification.addNotification(result.message || 'Gagal import', 'error')
    }
  } catch (error) {
    console.error(error)
    notification.addNotification('Terjadi kesalahan koneksi', 'error')
...
const downloadTemplate = async () => {
  ...
  try {
    const url = `/api/v1/supervisor/employee-data/karyawan-group/template?year=${props.activeYear}&month=${props.activeMonth}`
    ...
    let filename = 'Template_KaryawanGroup.xlsx'
    ...
    console.error(error)
    notification.addNotification('Gagal download template', 'error')
  } finally {
...
<template>
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
    <div class="bg-(--bg-card) w-full max-w-4xl rounded-md shadow-2xl border border-(--border-soft) overflow-hidden flex flex-col max-h-[90vh]">

      <!-- Header -->
      <div class="p-6 border-b border-(--border-soft) flex items-center justify-between shrink-0">
        <div>
          <h2 class="text-xl font-bold text-(--text-main)">Import Karyawan Group</h2>
          <p class="text-sm text-(--text-muted) mt-1">
            {{ step === 1 ? 'Upload file Excel/CSV berisi NIP karyawan' : 'Preview Data Import' }}
          </p>
        </div>
        ...
        <div class="bg-(--primary)/10 p-4 rounded-md border border-(--primary)/20 flex items-start gap-3">
          ...
          <div>
            ...
            <ul class="text-sm text-(--text-main) list-disc list-inside space-y-1">
              <li>Kolom A: <strong>NIP</strong> karyawan (wajib).</li>
              <li>Kolom B: Nama karyawan (opsional, untuk referensi).</li>
              <li>Karyawan dengan NIP valid akan dimasukkan ke group periode aktif.</li>
            </ul>
            <button @click="downloadTemplate" :disabled="isDownloading" class="mt-3 text-sm font-semibold text-(--primary) hover:underline flex items-center gap-1 disabled:opacity-50">
              ...
              <i v-else class="bx bx-download"></i> Download Template Excel
            </button>
          </div>
        </div>
      ...
      <div v-if="step === 2" class="p-6 space-y-4 flex-1 overflow-y-auto min-h-0">
        <div class="flex items-center gap-4 text-sm mb-2">
          <span class="text-(--text-main) font-bold">Periode: {{ periodName }}</span>
            ...
            <tbody class="divide-y divide-(--border-soft)">
              <tr v-for="item in previewData" :key="item.row" :class="!item.is_valid ? 'bg-(--danger)/5' : ''">
                ...
                <td class="px-4 py-3">
                  ...
                  <span v-else class="px-2 py-1 text-xs rounded-full bg-(--danger)/10 text-(--danger) font-semibold" :title="item.error">{{ item.error }}</span>
                </td>
              </tr>
              <tr v-if="previewData.length === 0">
                <td colspan="4" class="px-4 py-8 text-center text-(--text-muted)">Tidak ada data</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="p-6 border-t border-(--border-soft) flex justify-end gap-3 bg-(--bg-elevated) shrink-0">
        <button @click="closeModal" class="px-5 py-2.5 text-sm font-bold text-(--text-soft) hover:text-(--text-main) transition-colors">
          Batal
        </button>
