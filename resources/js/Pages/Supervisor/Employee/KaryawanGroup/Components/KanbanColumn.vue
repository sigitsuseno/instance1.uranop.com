...
const emit = defineEmits(['drop', 'dragstart', 'toggleSelection', 'selectAllInGroup'])
...
<template>
  <div class="flex flex-col h-full min-h-[400px]">
    ...
    <!-- Drop Zone -->
    <div
      class="flex-1 p-2 rounded-md border-2 border-dashed transition-all duration-300 overflow-y-auto max-h-[calc(100vh-350px)] custom-scrollbar"
      :class="[
        draggedEmployeeId
      ...
      @dragover.prevent
      @drop.prevent="emit('drop', category.value)"
    >
