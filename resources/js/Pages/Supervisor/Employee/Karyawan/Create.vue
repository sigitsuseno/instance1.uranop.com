...
async function submit() {
  ...
  try {
    ...
    const res = await post('/api/v1/supervisor/employee-data/karyawan', payload)
    ...
    if (familyMembers.value.length > 0) {
      for (const member of familyMembers.value) {
        if (member.name) {
          try {
            await post(`/api/v1/supervisor/employee-data/karyawan/${employee.id}/families`, member)
          } catch {}
        }
      }
    }
    ...
    router.push(`/supervisor/employee-data/karyawan/${employee.id}`)
  } catch (e) {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      <BaseButton variant="ghost" @click="router.push('/supervisor/employee-data/karyawan')">
        <template #icon-left>
