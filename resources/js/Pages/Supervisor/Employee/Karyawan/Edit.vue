...
async function loadDepartments() {
  ...
  try {
    const res = await get('/api/v1/supervisor/master/departments/options')
...
async function loadPositions(deptId) {
  ...
  try {
    const res = await get(`/api/v1/supervisor/master/positions/options?department_id=${deptId}`)
...
async function fetchEmployee() {
  try {
    const res = await get(`/api/v1/supervisor/employee-data/karyawan/${employeeId.value}`)
...
async function submit() {
  ...
  try {
    ...
    await put(`/api/v1/supervisor/employee-data/karyawan/${employeeId.value}`, payload)

    notification.success(`Data karyawan berhasil diperbarui.`)
    router.push(`/supervisor/employee-data/karyawan/${employeeId.value}`)
  } catch (e) {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center gap-3">
      <BaseButton variant="ghost" @click="router.push(`/supervisor/employee-data/karyawan/${employeeId}`)">
        <template #icon-left>
