...
async function fetchStats() {
  try {
    const res = await get('/api/v1/supervisor/employee-data/karyawan/stats')
...
async function fetchPositions() {
  try {
    const res = await get('/api/v1/supervisor/master/positions?per_page=100')
...
async function handleDelete() {
  ...
  try {
    await apiDelete(`/api/v1/supervisor/employee-data/karyawan/${selectedEmployee.value.id}`)
...
async function handleDeactivate() {
  ...
  try {
    await patch(`/api/v1/supervisor/employee-data/karyawan/${selectedEmployee.value.id}/deactivate`, deactivateForm.value)
...
async function exportData() {
  try {
    ...
    const response = await fetch(`/api/v1/supervisor/employee-data/karyawan/export?${params.toString()}`, {
        headers
    })
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <BaseButton variant="secondary" @click="$router.push('/supervisor/employee-data/karyawan/import')">
          <template #icon-left>
        ...
        <BaseButton variant="primary" @click="$router.push('/supervisor/employee-data/karyawan/create')" class="shadow-lg shadow-(--primary-glow)">
          <template #icon-left>
    ...
    <div v-else class="bg-(--bg-card) rounded-md border border-(--border-soft) p-16 text-center shadow-sm">
      ...
      <div class="mt-6 flex justify-center gap-3">
        ...
        <BaseButton variant="primary" @click="$router.push('/supervisor/employee-data/karyawan/create')">
          <template #icon-left>
            <i class="bx bx-plus text-lg"></i>
          </template>
          Tambah Baru
