...
async function fetchSalaries() {
  ...
  try {
    ...
    const res = await get(`/api/v1/supervisor/employee-data/salaries?${params}`)
...
function viewSalary(id) {
  router.push(`/supervisor/employee-data/gaji-karyawan/${id}`)
}

function editSalary(id) {
  router.push(`/supervisor/employee-data/gaji-karyawan/${id}/edit`)
}
...
async function handleDelete() {
  ...
  try {
    await apiDelete(`/api/v1/supervisor/employee-data/salaries/${selectedSalary.value.id}`)
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <BaseButton variant="secondary" @click="$router.push('/supervisor/employee-data/gaji-karyawan/import')">
          <template #icon-left><i class="bx bx-import text-lg"></i></template>
          Import
        </BaseButton>
        <BaseButton variant="primary" @click="$router.push('/supervisor/employee-data/gaji-karyawan/create')">
            ...
            <tr v-for="salary in salaries" :key="salary.id" class="hover:bg-(--bg-elevated)/50 transition-colors">
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex items-center justify-end gap-1">
                  <button @click="$router.push(`/supervisor/employee-data/gaji-karyawan/${salary.id}`)" class="w-8 h-8 rounded text-(--text-muted) hover:text-(--primary) hover:bg-(--bg-elevated) transition-all"><i class="bx bx-show"></i></button>
                  <button @click="$router.push(`/supervisor/employee-data/gaji-karyawan/${salary.id}/edit`)" class="w-8 h-8 rounded text-(--text-muted) hover:text-(--primary) hover:bg-(--bg-elevated) transition-all"><i class="bx bx-edit"></i></button>
