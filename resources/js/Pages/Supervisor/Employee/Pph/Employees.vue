...
async function fetchData() {
  ...
  try {
    ...
    const res = await api.get(`/api/v1/supervisor/employee-data/pph/employees?${query.toString()}`)
    employees.value = res
  } catch (err) {
...
async function toggleNpwp(emp) {
  try {
    ...
    await api.put(`/api/v1/supervisor/employee-data/pph/employees/${emp.id}`, payload);
...
async function saveSettings() {
  ...
  try {
    const res = await api.put(`/api/v1/supervisor/employee-data/pph/employees/${editingEmployee.value.id}`, form)
