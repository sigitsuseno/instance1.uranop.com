  ...
  <div class="py-2 space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        ...
        :key="card.label"
        class="bg-(--bg-card) border border-(--border-soft) rounded-md p-4 flex items-center gap-4"
      >
        <div class="w-10 h-10 shrink-0 rounded-md flex items-center justify-center" :class="card.iconBg">
          <component :is="card.icon" :class="['w-5 h-5', card.iconColor]" />
        </div>
        <div class="min-w-0">
          <p class="text-xs font-bold text-(--text-muted) uppercase tracking-widest truncate">{{ card.label }}</p>
          <div class="text-2xl font-extrabold text-(--text-main) leading-tight">{{ card.value }}</div>
          <p class="text-xs text-(--text-muted) mt-0.5 truncate">{{ card.sub }}</p>
        </div>
      </div>
    </div>
    ...
    <!-- Row 2: Kontrak, Aktivitas Terbaru, Ulang Tahun -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- Kontrak Akan Berakhir -->
      <BaseCard class="max-h-[500px] flex flex-col">
        <template #title>
          <div class="flex items-center gap-2">
        ...
        <div v-else class="space-y-3.5 flex-1 min-h-0 overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-(--border-soft) [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-(--text-soft) transition-all duration-300">
          <div
      ...
      <BaseCard class="max-h-[500px] flex flex-col">
        ...
        <div v-else class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
          <DataTable :headers="auditHeaders" :items="recentAuditLogs">
      ...
      <BaseCard class="max-h-[500px] flex flex-col">
        ...
        <div v-else class="space-y-3 flex-1 min-h-0 overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-(--border-soft) [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-(--text-soft) transition-all duration-300">
          <div
