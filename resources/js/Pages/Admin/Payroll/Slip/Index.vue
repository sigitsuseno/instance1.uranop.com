<template>
  <div class="space-y-6">
    ...
    <div v-if="records.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div class="bg-(--bg-card) border border-(--border-soft) rounded-md p-4 shadow-sm">
        <div class="text-xs text-(--text-muted) mb-1">Total Gaji Kotor</div>
        <div class="text-lg font-bold text-(--primary)">{{ formatCurrency(stats.total_gaji_kotor, true) }}</div>
      </div>
      <div class="bg-(--bg-card) border border-(--border-soft) rounded-md p-4 shadow-sm">
        <div class="text-xs text-(--text-muted) mb-1">Total Potongan</div>
        <div class="text-lg font-bold text-red-600">{{ formatCurrency(stats.total_potongan, true) }}</div>
      </div>
      <div class="bg-(--bg-card) border border-(--border-soft) rounded-md p-4 shadow-sm">
        <div class="text-xs text-(--text-muted) mb-1">Total Bersih</div>
        <div class="text-lg font-bold text-green-600">{{ formatCurrency(stats.total_bersih, true) }}</div>
      </div>
    </div>
    ...
    <BaseCard v-if="selectedPeriod" padding="p-0" class="overflow-hidden border border-(--border-soft) bg-(--bg-card) rounded-md shadow-sm">
      ...
      <div class="px-4 py-3 border-b border-(--border-soft) flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-(--bg-main)/30">
        <div>
          <h3 class="text-sm font-semibold text-(--text-main)">Daftar Slip Gaji</h3>
        ...
        <table class="w-full text-xs min-w-[960px]">
          ...
          <tbody>
            <tr v-if="filteredRecords.length === 0">
              <td colspan="12" class="px-4 py-12 text-center text-(--text-muted) text-sm bg-(--bg-card)">
                ...
                <div v-else>
                  Belum ada data slip gaji untuk periode ini. Pastikan data gaji sudah di-generate dan dikunci pada menu <strong>Gaji Karyawan</strong>.
                </div>
              </td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="border border-(--border-soft) px-2.5 py-2">
                <div class="font-semibold text-(--text-main)">{{ record.employee_name }}</div>
                <div class="text-[10px] text-(--text-muted)">{{ record.position }}</div>
              </td>
              ...
              <td class="border border-(--border-soft) px-2 py-2 text-right font-mono text-red-600">
                {{ formatCurrency(
                  record.bpjs_tk + record.bpjs_ks + record.bpjs_pen + record.pph + record.cashbon + record.pot_kehadiran
                ) }}
              </td>
    ...
    <BaseCard v-else class="py-16 border border-(--border-soft) bg-(--bg-card) rounded-md shadow-sm">
      <div class="text-center space-y-4 max-w-sm mx-auto">
        ...
        <div>
          ...
          <p class="text-sm text-(--text-muted) mt-1">Pilih periode dari dropdown di atas untuk melihat dan mencetak slip gaji karyawan</p>
        </div>
      </div>
    </BaseCard>
  </div>
...
const totals = computed(() => {
  ...
  return {
    ...
    gaji_kotor: fSum('gaji_kotor'),
    potongan: filteredRecords.value.reduce((acc, r) =>
      acc + (r.bpjs_tk + r.bpjs_ks + r.bpjs_pen + r.pph + r.cashbon + r.pot_kehadiran), 0),
...
async function fetchPayslips() {
  ...
  loading.value = true
  try {
    let url = `/api/v1/payroll/payslips?period_id=${selectedPeriodId.value}`
...
function openPrintWindow(html) {
  ...
  if (!win) {
    notification.error('Popup diblokir browser. Izinkan popup dan coba lagi.')
    return
  }
...
function buildNormalSlipData(r) {
  ...
  const hkAmount = hk * ratePerHari // = gaji_pokok (full)
  ...
  return {
    ...
    bpjsTK: r.bpjs_tk,
    bpjsKes: r.bpjs_ks,
    bpjsPensiun: r.bpjs_pen,
    pph: r.pph,
    cashbon: r.cashbon,
    deductDays: deductDays,
    potonganKehadiran: potKehadiran,
    pblt: r.pblt,
    subTotalDeduction: Math.round(subTotalDeduction),
    totalTerima: r.gaji_bersih,
  }
}
...
function buildSplitSlipData(r) {
  // Current segment is the primary
  ...
  return {
    ...
    part1,
    part2,
    totalTerima: (part1?.totalTerima || 0) + (part2?.totalTerima || 0),
  }
}
...
function generateBulkPrintHtml(slips, isSplitMode) {
  ...
  const renderNormalSlip = (s) => `<div class="slip">
<div class="sisa-cuti">Sisa Cuti: ${s.remainingLeave} hari</div>
<div class="co-header"><b class="co-name">PT. KEMILAU UNGARAN SUKSES</b><div class="co-sub">EMBROIDERY &amp; PRINTING FACTORY</div><div class="co-addr">Jl. Ngobo/Jl. PTPN IX No.1 Gudang Dolog BGR Karangjati, Bergas Kab. Smg 50552</div><div class="co-addr">(0298) 522686, 525052</div></div>
<div class="slip-title">SLIP GAJI</div>
<div class="emp-info"><span>${s.employeeName}</span><span>BULAN: ${periodName}<br>No ACCOUNT: ${s.employeeCode}</span></div>
<div class="tbl-hdr"><span>KETERANGAN PENDAPATAN</span><span>JUMLAH</span></div>
<div class="row b"><span>GAJI POKOK</span><span></span>${amt(s.hkAmount, {noDot: true})}</div>
<div class="row"><span class="lbl">HK</span><span>${s.hkDays} x Rp ${f(s.ratePerHari)}</span>${amt(s.hkAmount)}</div>
...
<div class="sp"></div>
<div class="row"><span class="lbl">BPJS TK</span><span></span>${amt(s.bpjsTK, {deduct: true})}</div>
<div class="row"><span class="lbl">BPJS KS</span><span></span>${amt(s.bpjsKes, {deduct: true})}</div>
<div class="row"><span class="lbl">BPJS PENSIUN</span><span></span>${amt(s.bpjsPensiun, {deduct: true})}</div>
<div class="row"><span class="lbl">PPH</span><span></span>${amt(s.pph, {deduct: true})}</div>
<div class="row"><span class="lbl">CASHBON</span><span></span>${amt(s.cashbon, {deduct: true})}</div>
<div class="row"><span class="lbl">POT. KEHADIRAN</span><span>${s.deductDays} HARI</span>${amt(s.potonganKehadiran, {deduct: true})}</div>
...
<div class="sp"></div>
${p.part === 2 ? `
<div class="row"><span class="lbl">BPJS TK</span><span></span>${amt(p.bpjsTK, {deduct: true})}</div>
<div class="row"><span class="lbl">BPJS KS</span><span></span>${amt(p.bpjsKes, {deduct: true})}</div>
<div class="row"><span class="lbl">BPJS PENSIUN</span><span></span>${amt(p.bpjsPensiun, {deduct: true})}</div>
<div class="row"><span class="lbl">PPH</span><span></span>${amt(p.pph, {deduct: true})}</div>
` : ''}
<div class="row"><span class="lbl">CASHBON</span><span></span>${amt(p.cashbon, {deduct: true})}</div>
<div class="row"><span class="lbl">POT. KEHADIRAN</span><span>${p.deductDays} HARI</span>${amt(p.potonganKehadiran, {deduct: true})}</div>
<div class="row xs"><span></span><span style="text-align:right">PBLT</span>${amt(p.pblt, {pblt: true})}</div>
<div class="row mb-1"><span></span><span></span>${amt(p.subTotalDeduction, {deduct: true, bt: true})}</div>
<div class="row part-total"><span></span><span>Sub Total</span>${amt(p.totalTerima)}</div>
    `
  }

  const renderSplitSlip = (s) => `<div class="slip slip-split">
<div class="sisa-cuti">Sisa Cuti: ${s.remainingLeave} hari</div>
<div class="co-header"><b class="co-name">PT. KEMILAU UNGARAN SUKSES</b><div class="co-sub">EMBROIDERY &amp; PRINTING FACTORY</div><div class="co-addr">Jl. Ngobo/Jl. PTPN IX No.1 Gudang Dolog BGR Karangjati, Bergas Kab. Smg 50552</div><div class="co-addr">(0298) 522686, 525052</div></div>
<div class="slip-title">SLIP GAJI</div>
<div class="emp-info"><span>${s.employeeName}</span><span>No ACCOUNT: ${s.employeeCode}</span></div>
<div class="tbl-hdr"><span>KETERANGAN PENDAPATAN</span><span>JUMLAH</span></div>
<div class="split-container">
    ${renderSplitPart(s.part1, 'PART 1')}
    <div class="sp border-b border-dashed border-gray-400 my-1"></div>
...
.page{width:210mm;display:grid;grid-template-columns:1fr 1fr;gap:2mm;padding:5mm;page-break-after:always}
.page:last-child{page-break-after:auto}
.slip{border:1px solid #000;padding:2mm;position:relative;font-size:7pt;overflow:hidden}
.slip-split { font-size: 6.5pt; }
.slip-split .row { font-size: 6.5pt; }
.sisa-cuti{position:absolute;top:0;right:0;border-left:1px solid #000;border-bottom:1px solid #000;font-size:6pt;padding:1px 3px}
.co-header{border-bottom:1px solid #000;padding-bottom:1mm;margin-bottom:1mm}
.co-name{font-size:8pt;text-transform:uppercase}
.co-sub{font-style:italic;font-size:6.5pt}
.co-addr{font-size:6pt;color:#555}
.slip-title{text-align:center;font-weight:bold;font-size:8pt;border-bottom:1px solid #000;margin-bottom:1mm}
.emp-info{display:grid;grid-template-columns:1fr 1fr;font-size:7pt;font-weight:500;padding:1mm 0;border-bottom:1px dashed #ccc;margin-bottom:1mm}
.tbl-hdr{display:flex;justify-content:space-between;border-top:1px solid #000;border-bottom:1px solid #000;font-weight:bold;font-size:7pt;padding:1px 0;margin-bottom:1mm}
.row{display:grid;grid-template-columns:2fr 1.5fr 1.5fr;font-size:7pt;line-height:1.3}
.row.b{font-weight:bold}
.row.xs{font-size:5.5pt}
.row .lbl::after{content:":";margin-left:2px}
.bt{border-top:1px solid #000}
.row.total{border-top:1px solid #000;border-bottom:1px solid #000;font-weight:bold;font-size:7.5pt}
.row.part-total{font-weight:bold;font-style:italic}
.split-total{font-size:8pt!important; background:#f0f0f0; padding:2px 0}
.amt{display:flex;justify-content:space-between;width:100%}
.sp{height:1.5mm}
.mb-1{margin-bottom:1mm}
.mt-1{margin-top:1mm}
.my-1{margin-top:1mm;margin-bottom:1mm}
.split-part-title{font-weight:bold;text-decoration:underline;margin:1mm 0;text-transform:uppercase}
.ftr-date{text-align:right;font-size:6pt;padding:1mm 1mm 0}
.ftr-sign{display:flex;gap:4mm;font-size:6pt;padding:0 3mm 1mm}
.sc{text-align:center;width:40%}
.sg{height:8mm}
@media print{
    body{margin:0}
