...
const uploadFile = async () => {
  ...
  try {
    const res = await post('/api/v1/employees/contracts/import', formData, {
      headers: {
...
<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
      <button @click="$router.push('/admin/employees/contracts')" class="w-10 h-10 rounded-full bg-(--bg-card) border border-(--border-soft) flex items-center justify-center text-(--text-soft) hover:text-(--primary) hover:border-(--primary)/50 transition-colors shadow-sm">
      ...
      <div class="md:col-span-2 space-y-4">
        ...
        <BaseCard v-if="uploadResult" class="border-(--border-soft) shadow-sm">
          ...
          <div class="mt-6 flex justify-end">
            <BaseButton variant="primary" @click="$router.push('/admin/employees/contracts')">
