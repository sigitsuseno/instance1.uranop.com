...
async function fetchStats() {
  try {
    const res = await get('/api/v1/employees/contracts/stats')
...
async function fetchContractHistory(employee) {
  ...
  try {
    const res = await get(`/api/v1/employees/${employee.id}/contracts`)
...
async function markCompensationPaid(contractId, employeeId) {
  try {
    await patch(`/api/v1/employees/${employeeId}/contracts/${contractId}/mark-paid`)
