  ...
  <div>
    ...
    <div class="bg-(--bg-card) border border-(--border-soft) rounded-xl shadow-sm p-4 mb-6">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <select v-model="selectedPeriod" class="px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-elevated) text-sm min-w-[280px]" @change="fetchList" :disabled="isLoading">
            <option v-for="p in payPeriods" :key="p.id" :value="p.id">{{ p.label }}</option>
          </select>
        </div>
        <div class="flex gap-2">
          <select v-model="filterType" class="px-3 py-1.5 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm" @change="fetchList">
        ...
        <tbody>
          <tr v-for="s in streaks" :key="s.id" class="border-b border-(--border-soft) hover:bg-(--bg-elevated)/30">
            ...
            <td class="px-4 py-3 text-center">
              <span class="px-2 py-0.5 text-xs rounded-full font-semibold" :class="s.type==='worked'?'bg-green-100 text-green-800':'bg-red-100 text-red-800'">
                {{ s.type==='worked'?'Hadir':'Absen' }}
              </span>
            </td>
            <td class="px-4 py-3 text-center">{{ s.start_date }}</td>
            <td class="px-4 py-3 text-center">{{ s.end_date }}</td>
            <td class="px-4 py-3 text-center font-bold" :class="s.total_days>=7?'text-red-600':''">{{ s.total_days }}</td>
            <td class="px-4 py-3 text-xs text-(--text-muted) max-w-[200px] truncate">{{ s.notes || '—' }}</td>
            <td class="px-4 py-3 text-center">
              <div class="flex items-center justify-center gap-1">
                <button class="p-1 rounded hover:bg-(--bg-elevated) text-(--text-soft) hover:text-(--primary)" title="Edit" @click="openEdit(s)"><svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                <button class="p-1 rounded hover:bg-red-50 text-(--text-soft) hover:text-red-600" title="Hapus" @click="handleDelete(s)"><svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
              </div>
            </td>
          </tr>
    ...
    <div class="mt-4 text-sm text-(--text-muted) text-center" v-if="pagination.lastPage>1">
      <div class="flex items-center justify-center gap-2">
        <button :disabled="pagination.currentPage<=1" @click="goToPage(pagination.currentPage-1)" class="px-3 py-1 border rounded disabled:opacity-30">←</button>
        <span>Hlm {{ pagination.currentPage }}/{{ pagination.lastPage }}</span>
        <button :disabled="pagination.currentPage>=pagination.lastPage" @click="goToPage(pagination.currentPage+1)" class="px-3 py-1 border rounded disabled:opacity-30">→</button>
      </div>
    </div>

    <!-- Add/Edit Modal -->
    <BaseModal v-if="showModal" :show="showModal" :title="editingId ? 'Edit Consecutive Day' : 'Tambah Consecutive Day'" size="md" @close="closeModal">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium mb-1">Karyawan</label>
          <select v-model="form.employee_id" :disabled="!!editingId" class="w-full px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm" required>
            <option value="">— Pilih Karyawan —</option>
            <option v-for="e in employeeOptions" :key="e.id" :value="e.id">{{ e.employee_code }} — {{ e.name }}</option>
          </select>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-sm font-medium mb-1">Dari Tanggal</label>
            <input v-model="form.start_date" type="date" class="w-full px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm" required />
          </div>
          <div>
            <label class="block text-sm font-medium mb-1">Sampai Tanggal</label>
            <input v-model="form.end_date" type="date" class="w-full px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm" required />
          </div>
        </div>
        ...
        <div>
          ...
          <select v-model="form.type" class="w-full px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm">
            <option value="worked">Hadir</option>
            <option value="absent">Absen</option>
          </select>
        </div>
        <div>
          ...
          <textarea v-model="form.notes" rows="2" class="w-full px-3 py-2 border border-(--border-soft) rounded-lg bg-(--bg-card) text-sm" placeholder="Opsional..."></textarea>
        </div>
      </div>
      <template #footer>
        <div class="flex gap-3 w-full">
          ...
          <BaseButton variant="primary" :loading="isSaving" @click="handleSave">{{ editingId ? 'Simpan' : 'Tambah' }}</BaseButton>
        </div>
      </template>
    </BaseModal>

    <!-- Confirm Delete -->
    <BaseModal v-if="deletingId" :show="!!deletingId" title="Hapus Consecutive Day" size="sm" @close="deletingId=null">
...
async function fetchEmployeeOptions() {
  try {
    ...
    const res = await get(`/api/v1/employees?period_start=${start}&period_end=${end}&per_page=100`)
    employeeOptions.value = (res.data?.data || res.data || []).map(e=>({id:e.id,name:e.name,employee_code:e.employee_code}))
  } catch(e){console.error(e)}
}
