<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-(--text-main)">Laporan Payroll Karyawan</h1>
        <p class="text-sm text-(--text-muted) mt-1">Laporan rincian penggajian karyawan per periode</p>
      </div>
    ...
    <template v-if="selectedPeriod && sections.length > 0">
      ...
      <BaseCard padding="p-0" class="overflow-hidden border border-(--border-soft) bg-(--bg-card) rounded-md shadow-sm">
        <div class="px-4 py-3 border-b border-(--border-soft) flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-(--bg-main)/30">
          <div>
            <h3 class="text-sm font-semibold text-(--text-main)">Rincian Laporan Gaji Karyawan</h3>
...
tr:nth-child(odd) .sticky-col-last {
  background-color: rgba(var(--bg-main-rgb), 0.4);
}
.hover-row:hover .sticky-col,
.hover-row:hover .sticky-col-last {
