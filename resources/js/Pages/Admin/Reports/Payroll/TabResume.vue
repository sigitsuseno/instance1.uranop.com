    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
      <div class="flex items-center gap-3">
        ...
        <select v-model="groupTab" class="bg-(--bg-elevated) border border-(--border-soft) text-(--text-main) text-sm rounded-lg p-2 focus:ring-(--primary) focus:border-(--primary)">
          <option value="all-in">A. ALL IN</option>
          <option value="print">B. BULANAN PRINT</option>
        </select>
      ...
      <div v-else class="overflow-x-auto max-h-[65vh]">
        <table class="min-w-full divide-y divide-(--border-soft) text-xs whitespace-nowrap">
          ...
          <tbody class="divide-y divide-(--border-soft)">
            <tr v-for="(item, index) in data" :key="item.bagian" class="hover:bg-(--bg-elevated) transition-colors">
...
async function fetchData() {
  ...
  try {
    ...
    const res = await get(`/api/v1/laporan/payroll/resume?${params.toString()}`)
    data.value = res.data || []
  } catch (err) {
    notification.addNotification('Gagal mengambil data resume payroll', 'error')
  } finally {
    loading.value = false
  }
}
...
function exportExcel() {
  ...
  fetch(url, { headers: { 'Authorization': `Bearer ${token}` } })
    ...
    .then(blob => {
      ...
      link.setAttribute('download', `Laporan_Resume_${groupTab.value}.xlsx`)
