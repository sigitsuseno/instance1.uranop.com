...
async function fetchData() {
  ...
  try {
    ...
    const res = await get(`/api/v1/reports/lembur/resume?${params.toString()}`)
    data.value = res.data || []
...
function exportExcel() {
  ...
  const params = new URLSearchParams({ period_id: selectedPeriodId.value })
  props.groups.forEach(g => params.append('groups[]', g))
  const url = `/api/v1/reports/lembur/resume/export?${params.toString()}`
  fetch(url, { headers: { 'Authorization': `Bearer ${token}` } })
    ...
    .then(blob => {
      ...
      link.setAttribute('download', `Resume_Overtime_${safeName}.xlsx`)
...
function openPrint() {
  ...
  const params = new URLSearchParams({ period_id: selectedPeriodId.value })
  props.groups.forEach(g => params.append('groups[]', g))
  const url = `/api/v1/reports/lembur/resume/print?${params.toString()}`
  fetch(url, { headers: { 'Authorization': `Bearer ${token}` } })
    .then(r => r.text())
    .then(html => {
